...
class TopBottomRSimilarity(TopRSimilarity):
    ...
    def _init(inst, to_columns):
        ...
        for (column1_order, column2_order), label in zip(columns_tuples, labels):
            ...
            for r in r_range:
                top_r = inst._top_r_similarity(inst._context, inst._similarity, column1_order, column2_order, r)
                bottom_r = inst._top_r_similarity(inst._context, inst._similarity, column1_order_reversed, column2_order_reversed, r)
                results.append(
                    [
                        r,
                        top_r * bottom_r,
